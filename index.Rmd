...
nb_ranks <- x %>% 
  ...
  mutate(faculty_rank = case_when(str_detect(`PI NAME`, "AKYUZ,|BOXER,|CHEN,|CHEADLE,|DAVIS,|GIRASOLE,HRVATIN,|KALISH,|MEYER,|POLLINA,|SANTIAGO,|SHRESTHA,|SITKO,|WILSON,") ~ "Postdoc",
...
z %>% 
  ...
  rowwise() %>% 
  mutate(avg = mean(nih_2020, nih_2019, nih_2018, nih_2017, nih_2016)) %>% 
  filter(harvard_equivalent == "Nb") %>% 
  ungroup() %>% 
  arrange(desc(avg))  %>% 
  slice(1:10) %>% 
...
x %>% 
  ...
  labs(title = "NIH Funding for Harvard Neurobiology PIs",
       x = "Dollars", 
       y = "",
       fill = "Rank"), 
tooltip = "text")

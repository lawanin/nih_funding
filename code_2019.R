...
y %>% 
  ...
  mutate(nih = case_when(str_detect(nih, "CLEVELAND CLINIC LERNER") ~ "CASE WESTERN RESERVE UNIV/CLEVELAND CLINIC LERNER", 
                         ...
                         str_detect(nih, "RUTGERS BIOMEDICAL") ~ "RUTGERS, THE STATE UNIVERSITY OF NEW JERSEY",
...
# same command as for 2020. All 16 were applicable to 2019. 
...
departments_funding_2019 <- x_cleaned %>% 
  ...
  rename(nih_2019 = `TOTAL FUNDING`) 

full_nih_2019 <- x_cleaned %>% 
  rename(school = `ORGANIZATION NAME`,
         department_name = `DEPT NAME`) %>% 
  left_join(z, ., by = c("school", "department_name"))

write_csv(departments_funding_2019, "departments_nih_2019.csv")
write_csv(full_nih_2019, "full_nih_2019.csv")
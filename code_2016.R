...
y <- read_delim("source_files/Worldwide_2016.csv", skip = 1, delim = ";")
...
y %>% 
  ...
  # 2017 SCRB listed under FAS not HMS
  ...
  mutate(nih = case_when(str_detect(nih, "CLEVELAND CLINIC LERNER") ~ "CASE WESTERN RESERVE UNIV/CLEVELAND CLINIC LERNER", 
                         ...
                         # modification to line below for 2017 and following
                         
                         str_detect(nih, "MICHIGAN.*ANN|OF MICHIGAN$") ~ "UNIVERSITY OF MICHIGAN ANN ARBOR",
                         ...
                         # for 2017
                         # modification to line below for 2016 and following
                         str_detect(nih, "H.*SAN ANTONIO|UNIVERSITY OF TEXAS.*(CTR$|CENTER$)") ~ "UNIVERSITY OF TEXAS HLTH SCI CTR SAN ANTONIO",
                         str_detect(nih, "AUSTIN") ~ "UNIVERSITY OF TEXAS AUSTIN",
                         # because of HARVARD UNIVERSITY in SCRB
                         str_detect(nih, "HARVARD") ~ "HARVARD MEDICAL SCHOOL",
                         #for 2016
                         str_detect(nih, "GALVESTON") ~ "UNIVERSITY OF TEXAS MED BR GALVESTON",
                         str_detect(nih, ".*") ~ nih)) %>% 
  left_join(names, .) %>% 
  select(-nih) %>% 
...
x_cleaned <- x %>%  
  ...
  # trailing space in Harvard GHSM 2017
  mutate(`DEPT NAME` = str_replace_all(`DEPT NAME`, " $", ""))

departments_funding_2016 <- x_cleaned %>% 
  ...
  rename(nih_2016 = `TOTAL FUNDING`) 

full_nih_2016 <- x_cleaned %>% 
  rename(school = `ORGANIZATION NAME`,
         department_name = `DEPT NAME`) %>% 
  left_join(z, ., by = c("school", "department_name"))

write_csv(departments_funding_2016, "departments_nih_2016.csv")
write_csv(full_nih_2016, "full_nih_2016.csv")
...
y <- read_delim("source_files/Worldwide_2017.csv", skip = 1, delim = ";")
...
y %>% 
  ...
  mutate(nih = case_when(str_detect(nih, "CLEVELAND CLINIC LERNER") ~ "CASE WESTERN RESERVE UNIV/CLEVELAND CLINIC LERNER", 
                         ...
                         # for 2017
                         str_detect(nih, "H.*SAN ANTONIO") ~ "UNIVERSITY OF TEXAS HLTH SCI CTR SAN ANTONIO",
                         str_detect(nih, "AUSTIN") ~ "UNIVERSITY OF TEXAS AUSTIN",
...
departments_funding_2017 <- x_cleaned %>% 
  ...
  rename(nih_2017 = `TOTAL FUNDING`) 

full_nih_2017 <- x_cleaned %>% 
  rename(school = `ORGANIZATION NAME`,
         department_name = `DEPT NAME`) %>% 
  left_join(z, ., by = c("school", "department_name"))

write_csv(departments_funding_2017, "departments_nih_2017.csv")
write_csv(full_nih_2017, "full_nih_2017.csv")